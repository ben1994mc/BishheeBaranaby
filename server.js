...
// the quick and dirty trick which prevents crashing.
        ...
        console.log("POST");
                ...
                console.log("Body: " + body);
                        ...
                        console.log(customerNo + "test 1");
                        console.log(bookingRef + "test 2");
        ...
        console.log("POST");
                ...
                console.log("Body: " + body);
                    ...
                    const textTest = "SELECT * FROM hotelbooking.customer WHERE customer.c_no = $1";
                    const values1 = [customerNo];
                    const test1 = await client.query(textTest, values1);
                    
                    if(test1.rows = 0){
                    const text = "INSERT INTO hotelbooking.customer (c_no, c_name, c_email, c_address, c_cardtype, c_cardexp, c_cardno)"
                        + "VALUES($1, $2, $3, $4, $5, $6, $7);"

                        const values = [customerNo, json.Name, json.Email, Address, json.cardType, expiryDate, json.CardNo];
                        console.log(text, values)
                        // here we execute the data insertion command
                        const res1 = await client.query(text, values);
                            }
        ...
        console.log("POST");
                ...
                console.log("Body: " + body);
                        ...
                        const test1 = "SELECT * FROM hotelbooking.booking WHERE booking.b_ref = $1";
                        const testValues = [bookingRef];
                        const restest = await client.query(test1, testValues);
                                                
                        if(restest.rows > 0){
                            
                         const update = "update hotelbooking.booking set b_cost=(SELECT b_cost FROM hotel.booking WHERE b_ref=$1)+(SELECT DISTINCT(SELECT price FROM hotelbooking.rates WHERE r_class = $2)*"
                        ...
                        const restest = await client.query(update, updateValues);   
                         ...
                         const update2 = "update hotelbooking.booking set b_outstanding=(SELECT b_outstanding FROM hotel.booking WHERE b_ref=$1)+((SELECT DISTINCT(SELECT price FROM hotelbooking.rates WHERE r_class = $2)*"
                                       +" ((CAST($3 AS DATE))-(CAST($4 AS DATE))) FROM hotelbooking.rates)*0.9) "
                                       +" WHERE b_ref= $1;"
                        const updateValues2 = [bookingRef, room, json.COD, json.CID];
                        ...
                        + " (SELECT DISTINCT(SELECT price FROM hotelbooking.rates WHERE r_class = $2)*"
                        + " ((CAST($3 AS DATE))-(CAST($4 AS DATE))) FROM hotelbooking.rates),"
                        + " ((SELECT DISTINCT(SELECT price FROM hotelbooking.rates WHERE r_class = $2)*"
                        + " ((CAST($3 AS DATE))-(CAST($4 AS DATE))) FROM hotelbooking.rates)*0.9), 'no notes')"
                
                
                        const values = [bookingRef, room, json.COD, json.CID];
                        const res1 = await client.query(text, values);
                    }
        ...
        case '/add_room':
            ...
            if (req.method == 'POST') {
                ...
                req.on('end', async function () {
                   ...
                   const text = "(SELECT COALESCE(MAX(c_no),0)+1 FROM hotelbooking.customer)";                  
                    const res1 = await client.query(text); 
                    
                    const text2 = "SELECT COALESCE(MAX(b_ref),0)+1 FROM hotelbooking.booking";
                    ...
                    var json_str_new = JSON.stringify(json);
                    var json_str_new2 = JSON.stringify(json2);
                    
           
                    
                    var Acno = json_str_new.split(':');
                    customerNo = parseFloat(Acno[1]);
                    console.log(customerNo);
                    
                    var Abref = json_str_new2.split(':');
                    bookingRef = parseFloat(Abref[1]);
